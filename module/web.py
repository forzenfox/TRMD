...
from module.utils.stdio import PanelTable
from module.language import _t
from module.enums import WebMeta, ENVIRON, KeyWord
from module.utils.helpers import gen_random_credential, get_subprocess_args
...
class Web(TTYD, TMUX):
    ...
    def get_free_port():
        ...
        try:
            ...
            log.warning(
                f'无法使用{port}端口,已自动分配新的端口,{_t(KeyWord.REASON)}:"{e}"'
            )
            return _get_port(0)
    ...
    def run_once(self):
        ...
        log.info(
            f"通过浏览器运行,父进程pid:{env.get(ENVIRON.TRMD_WEB_PID)},未写入系统环境变量。"
        )
        cmd: list = [
            self.ttyd_path,
            "--writable",
        ...
        process = subprocess.Popen(
            cmd, env=env, stdout=file_handler.stream, stderr=file_handler.stream
        )
        ...
        log.info(
            f"通过浏览器运行,子进程pid:{os.environ.get(ENVIRON.TRMD_WEB_PID)},已写入系统环境变量。"
        )
    ...
    def run_session(self):
        ...
        if self.platform == "Windows":
            ...
            result = subprocess.run(
                [self.tmux_path, "has-session", "-t", session_name], capture_output=True
            )
            ...
            if result.returncode == 0:
                ...
                # 会话不存在，创建一个简单的会话。
                cmd.extend(
                    [self.tmux_path, "new-session", "-s", session_name, "cmd", "/k"]
                )
        ...
        try:
            # 根据平台使用不同的环境变量。
            use_env = (
                cmd_env if self.platform == "Windows" and "cmd_env" in locals() else env
            )
            process = subprocess.Popen(
                cmd, env=use_env, stdout=file_handler.stream, stderr=file_handler.stream
            )
            os.environ[ENVIRON.TRMD_WEB_PID] = str(process.pid)
            PanelTable(
                title="Web配置",
