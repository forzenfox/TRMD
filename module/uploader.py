...
from module.stdio import MetaData, ProgressBar
...
class TelegramUploader:
    def __init__(self, download_object):
        ...
        UploadTask.DIRECTORY_NAME = os.path.join(
            UploadTask.DIRECTORY_NAME, str(download_object.my_id)
        )
    ...
    async def resume_upload(
        ...
        for part_index in missing_parts:
            try:
                ...
                await self.client.save_file(
                    path=path, file_id=file_id, file_part=part_index
                )
                # 更新上传记录。
                ...
                if progress:
                    ...
                    func = partial(progress, current_size, file_size, *progress_args)
        ...
        if len(upload_task.file_part) != file_total_parts:
            raise Exception(
                f"分片上传不完整:{len(upload_task.file_part)}/{file_total_parts}"
            )

        is_big = file_size > 10 * 1024 * 1024
        if is_big:
            file = raw.types.InputFileBig(
                id=file_id, parts=file_total_parts, name=os.path.basename(path)
            )
        else:
        ...
        mime_type = self.client.guess_mime_type(file_path) or get_mime_from_extension(
            file_path
        )
        ...
        if file_path.lower().endswith((".jpg", ".jpeg", ".png", ".gif", ".webp")):
            try:
                media = raw.types.InputMediaUploadedPhoto(file=file, spoiler=False)
            ...
            if file_path.lower().endswith((".mp4", ".mov", ".avi", ".mkv")):
                ...
                if video_meta:
                    ...
                    console.log(p, style="#FF4689")
            media = await self.get_input_media_document(
                chat_id=chat_id, file=file, attributes=attributes, mime_type=mime_type
            )
            ...
            try:
                ...
                if upload_task.is_media_group and upload_task.send_as_media_group:
                    try:
                        ...
                        # 以message_id为键存储。
                        media_group_cache[media_group_id][message_id] = (
                            raw.types.InputSingleMedia(
                                media=media,
                                random_id=self.client.rnd_id(),
                                **await utils.parse_text_entities(
                                    self.client, text="", parse_mode=None, entities=None
                                ),
                            )
                        )
                        ...
                        log.info(
                            f'[Upload Worker]处理媒体组时出错,回退到单条发送,{_t(KeyWord.REASON)}:"{e}"'
                        )
                        # 出错时回退到单条发送。
                        await self.send_media(media, upload_task)
                ...
                log.error(
                    f'[Upload Worker]错误,{_t(KeyWord.REASON)}:"{e}"', exc_info=True
                )
            finally:
                    ...
                    if media_group_id in media_group_cache:
                        ...
                        for message in media_group:
                            ...
                            if (
                                ...
                                and msg_id in media_group_cache[media_group_id]
                            ):
                                sorted_media_group.append(
                                    media_group_cache[media_group_id][msg_id]
                                )
                        ...
                        if sorted_media_group:
                            ...
                            try:
                                ...
                                for task in UploadTask.TASKS:
                                    if (
                                        task.message_id in message_ids
                                        and task.status == UploadStatus.SUCCESS
                                    ):
                                        task.status = UploadStatus.SENT
                                self.valid_link_cache = {
                                    k: v
                                    for k, v in self.valid_link_cache.items()
                                    if v != chat_id
                                }
                            except Exception as send_error:
                                log.error(
                                    f'[Upload Worker]发送媒体组失败,{_t(KeyWord.REASON)}:"{send_error}"',
                                    exc_info=True,
                                )
                        else:
                            log.warning(
                                f'[Upload Worker]发送媒体组"{media_group_id}"没有可发送的媒体。'
                            )
    ...
    async def send_media(
        self, media: raw.types.InputMediaDocument, upload_task: UploadTask
    ):
        """发送单条媒体消息。"""
        try:
            chat_id = upload_task.chat_id
            ...
            self.valid_link_cache = {
                k: v for k, v in self.valid_link_cache.items() if v != chat_id
            }
            log.info(
                f'[Upload Worker]单条消息发送完成,{_t(KeyWord.CHANNEL)}:"{chat_id}"'
            )
        except Exception as e:
            log.error(
                f'"[Upload Worker]发送单条消息失败,{_t(KeyWord.REASON)}:"{e}"',
                exc_info=True,
            )
    ...
    async def create_upload_task(
        self, link: Union[str, int], upload_task: UploadTask
    ) -> None:
        ...
        while retry < self.max_upload_retries:
            try:
                await self.__add_task(upload_task=upload_task)
                return None
    ...
    async def __add_task(self, upload_task: UploadTask):
        file_path = upload_task.file_path
        file_size = upload_task.file_size
        while (
            self.current_task_num >= self.max_upload_task
        ):  # v1.0.7 增加下载任务数限制。
            await self.event.wait()
            self.event.clear()
        upload_task.status = UploadStatus.UPLOADING
        console.log(
            f'{_t(KeyWord.UPLOAD_TASK)}{_t(KeyWord.RESUME)}:"{file_path}"。'
        ) if upload_task.file_part else None
        ...
        _task = self.loop.create_task(
            self.resume_upload(
                ...
                progress_args=(self.pb.progress, task_id, upload_task),
            )
        )
        _task.add_done_callback(
            partial(self.upload_complete_callback, upload_task, task_id)
        )

        if _task:
            self.current_task_num += 1
            MetaData.print_current_task_num(
                prompt=_t(KeyWord.CURRENT_UPLOAD_TASK), num=self.current_task_num
            )
            await _task

    def upload_complete_callback(self, upload_task, task_id, _future):
        try:
            _ = _future.result()
        except Exception as e:
            self.current_task_num -= 1
        ...
        if upload_task.file_size < 10 * 1024 * 1024:
            if not safe_delete(
                os.path.join(UploadTask.DIRECTORY_NAME, f"{upload_task.sha256}.json")
            ):
                log.warning(
                    f'无法删除"{os.path.basename(file_path)}"的上传缓存管理文件。'
                )
            else:
        ...
        if (
            ...
            and str(upload_task.chat_id)
            == self.repository_manager.get_repository_chat_id()
            and hasattr(upload_task, "source_chat_id")
            and upload_task.source_chat_id is not None
            and hasattr(upload_task, "source_message_id")
            and upload_task.source_message_id is not None
        ):
            try:
                asyncio.create_task(self._repository_on_upload_success(upload_task))
            except Exception as e:
                log.warning(f"仓库记录写入调度失败: {e}")

    async def _repository_on_upload_success(self, upload_task):
        """仓库模式上传成功回调（异步执行）。

        从仓库频道获取上传后的消息，调用 repository_manager.on_upload_success 写入记录。
    ...
    def download_upload(self, with_upload: dict, file_path: str):
        if isinstance(with_upload, dict):
            # 去重命中时：直接从仓库分发到目标频道，无需上传本地文件
            if with_upload.get("_dedup_hit") or with_upload.get("_dedup_l1_hit"):
                file_unique_id = with_upload.get("_dedup_file_unique_id")
                target_link = with_upload.get("link")
                if (
                    file_unique_id
                    and target_link
                    and self.repository_manager is not None
                    and self.repository_manager.should_use_repository()
                ):
                    asyncio.create_task(
                        self._dedup_distribute(
            ...
            asyncio.create_task(
                self.create_upload_task(
                    ...
                    upload_task=UploadTask(
                        ...
                        send_as_media_group=with_upload.get(
                            "send_as_media_group", False
                        ),
                        source_chat_id=with_upload.get("source_chat_id"),
                        source_message_id=with_upload.get("source_message_id"),
                    ),
                )
            )

    async def _dedup_distribute(
        self,
        file_unique_id: str,
        target_link: str,
        with_upload: dict,
        ...
        Args:
            file_unique_id: 仓库中已有文件的 file_unique_id
            target_link: 目标频道链接
            with_upload: 上传配置字典
        """
        try:
            target_meta = await parse_link(
                client=self.client,
                link=target_link,
            )
            if not target_meta:
                log.warning(f"去重分发失败: 无法解析目标链接 {target_link}")
                return
            target_chat_id = target_meta.get("chat_id")
            if not target_chat_id:
                log.warning("去重分发失败: 目标频道 ID 为空")
                return
            result_msg_id = await self.repository_manager.distribute_to_target(
                client=self.client,
                file_unique_id=file_unique_id,
            ...
            if result_msg_id is not None:
                log.info(
                    f"去重分发成功: file_unique_id={file_unique_id} -> target={target_chat_id}:{result_msg_id}"
                )
            else:
                log.warning(
                    f"去重分发失败: file_unique_id={file_unique_id} -> target={target_chat_id}，所有分发方式均失败"
                )
        except Exception as e:
            log.warning(f"去重分发异常: {e}")
