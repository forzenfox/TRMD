...
class ConfigManager:
    ...
    def validate_config(self, config: dict) -> tuple[bool, list[str]]:
        ...
        if "resource_limits" in config:
            ...
            # 并发数
            for key in ["max_concurrent_tasks", "max_download_concurrency",
                        "max_upload_concurrency", "max_forward_concurrency"]:
                val = rl.get(key)
                if val is not None:
    ...
    def validate_repository_config(self) -> tuple[bool, str]:
        ...
        if enabled:
            ...
            if not re.match(r'^-?\d+$', str(chat_id)):
                return (False, f"repository.chat_id 格式无效: '{chat_id}'，应为数字字符串")
        ...
        if auto_sync_enabled:
            ...
            if not isinstance(interval, (int, float)) or interval <= 0:
                return (False, "repository.auto_sync_interval_minutes 必须为正数")
