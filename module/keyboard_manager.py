...
class KeyboardManager:
    ...
    def build_download_setting_keyboard(download_type: list) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_ON if DownloadType.VIDEO in download_type else BotButton.VIDEO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_VIDEO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.PHOTO_ON if DownloadType.PHOTO in download_type else BotButton.PHOTO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_PHOTO
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.AUDIO_ON if DownloadType.AUDIO in download_type else BotButton.AUDIO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_AUDIO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.VOICE_ON if DownloadType.VOICE in download_type else BotButton.VOICE_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_VOICE
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.ANIMATION_ON if DownloadType.ANIMATION in download_type else BotButton.ANIMATION_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_ANIMATION
                    ),
                    InlineKeyboardButton(
                        text=BotButton.DOCUMENT_ON if DownloadType.DOCUMENT in download_type else BotButton.DOCUMENT_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_DOCUMENT
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_NOTE_ON if DownloadType.VIDEO_NOTE in download_type else BotButton.VIDEO_NOTE_OFF,
    ...
    def build_forward_setting_keyboard(forward_type: dict) -> InlineKeyboardMarkup:
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_ON if forward_type.get('video') else BotButton.VIDEO_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_VIDEO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.PHOTO_ON if forward_type.get('photo') else BotButton.PHOTO_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_PHOTO
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.AUDIO_ON if forward_type.get('audio') else BotButton.AUDIO_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_AUDIO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.VOICE_ON if forward_type.get('voice') else BotButton.VOICE_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_VOICE
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.ANIMATION_ON if forward_type.get('animation') else BotButton.ANIMATION_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_ANIMATION
                    ),
                    InlineKeyboardButton(
                        text=BotButton.DOCUMENT_ON if forward_type.get('document') else BotButton.DOCUMENT_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_DOCUMENT
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.TEXT_ON if forward_type.get('text') else BotButton.TEXT_OFF,
                        callback_data=BotCallbackText.TOGGLE_FORWARD_TEXT
                    ),
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_NOTE_ON if forward_type.get('video_note') else BotButton.VIDEO_NOTE_OFF,
    ...
    def build_export_table_toggle_keyboard(
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=BotButton.CLOSE_LINK_TABLE if config.get('export_table').get('link') else BotButton.OPEN_LINK_TABLE,
                        callback_data=BotCallbackText.TOGGLE_LINK_TABLE
                    ),
                    InlineKeyboardButton(
                        text=BotButton.CLOSE_COUNT_TABLE if config.get('export_table').get('count') else BotButton.OPEN_COUNT_TABLE,
                        callback_data=BotCallbackText.TOGGLE_COUNT_TABLE
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.CLOSE_UPLOAD_TABLE if config.get('export_table').get('upload') else BotButton.OPEN_UPLOAD_TABLE,
    ...
    @staticmethod
    def build_download_chat_filter_keyboard(include_comment: bool) -> InlineKeyboardMarkup:
        """构建下载频道过滤设置键盘。"""
        return InlineKeyboardMarkup(
            [
                [
    ...
    def build_download_chat_dtype_filter_keyboard(
        ...
        return InlineKeyboardMarkup(
            [
                [
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_ON if dtype[DownloadType.VIDEO] else BotButton.VIDEO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_VIDEO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.PHOTO_ON if dtype[DownloadType.PHOTO] else BotButton.PHOTO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_PHOTO
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.AUDIO_ON if dtype[DownloadType.AUDIO] else BotButton.AUDIO_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_AUDIO
                    ),
                    InlineKeyboardButton(
                        text=BotButton.VOICE_ON if dtype[DownloadType.VOICE] else BotButton.VOICE_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_VOICE
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.ANIMATION_ON if dtype[DownloadType.ANIMATION] else BotButton.ANIMATION_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_ANIMATION
                    ),
                    InlineKeyboardButton(
                        text=BotButton.DOCUMENT_ON if dtype[DownloadType.DOCUMENT] else BotButton.DOCUMENT_OFF,
                        callback_data=BotCallbackText.TOGGLE_DOWNLOAD_CHAT_DTYPE_DOCUMENT
                    )
                ],
                [
                    InlineKeyboardButton(
                        text=BotButton.VIDEO_NOTE_ON if dtype[DownloadType.VIDEO_NOTE] else BotButton.VIDEO_NOTE_OFF,
...
class KeyboardButtonHandler:
    ...
    async def handle_choice_export_table(self, choice: Union[BotCallbackText, str]) -> None:
        """处理导出表格选择按钮点击。"""
    ...
    async def handle_toggle_setting(self, global_config: dict, user_config: dict) -> None:
        """处理设置按钮切换。"""
        try:
            await self.callback_query.message.edit_reply_markup(
    ...
    async def handle_toggle_download_chat_dtype_filter(self, download_chat_filter: dict, chat_id: str) -> None:
        """处理下载频道类型过滤按钮切换。"""
        try:
            await self.callback_query.message.edit_reply_markup(
                KeyboardManager.build_download_chat_dtype_filter_keyboard(download_chat_filter, chat_id)
            )
        except MessageNotModified:
            pass
    ...
    async def handle_toggle_table(self, config: dict, choice: Union[str, None] = None) -> None:
        """处理表格导出开关按钮切换。"""
        try:
            await self.callback_query.message.edit_reply_markup(
