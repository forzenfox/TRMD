...
class TestTaskItemModel:
    ...
    def test_task_item_can_retry(self):
        """测试可重试判定。"""
        # FloodWait 可重试
        item1 = TaskItem(item_id='msg_100', status=ItemStatus.FAILED, error_reason='FloodWait')
        assert item1.can_retry() is True

        # 网络超时 可重试
        item2 = TaskItem(item_id='msg_101', status=ItemStatus.FAILED, error_reason='TimeoutError')
        assert item2.can_retry() is True

        # 消息已删除 不可重试
        item3 = TaskItem(item_id='msg_102', status=ItemStatus.FAILED, error_reason='MESSAGE_ID_INVALID')
        assert item3.can_retry() is False

        # 无权限 不可重试
        item4 = TaskItem(item_id='msg_103', status=ItemStatus.FAILED, error_reason='CHAT_FORBIDDEN')
...
class TestTaskItemManagement:
    ...
    async def test_update_item_status(self, task_manager):
        ...
        await task_manager.update_item_status(
            task.task_id, 'msg_1', ItemStatus.SUCCESS
        )
        fetched_task = await task_manager.get_task(task.task_id)
